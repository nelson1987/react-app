# Passo 7: Gerar APK de Produção (Linux)
...
### Copiar APK via ADB (Linux específico):

```bash
# Copiar direto para o celular
adb push android/app/build/outputs/apk/debug/app-debug.apk /sdcard/Download/

# Abrir gerenciador de arquivos no celular
...
#### Mover keystore para local seguro:
...
# Mover keystore
mv meu-app-key.keystore ~/keystores/

# Fazer backup
cp ~/keystores/meu-app-key.keystore ~/keystores/meu-app-key.keystore.backup
```
...
android {
    ...
    signingConfigs {
        release {
            storeFile file('~/keystores/meu-app-key.keystore')
...
```properties
MYAPP_RELEASE_STORE_FILE=~/keystores/meu-app-key.keystore
...
💡 **No Linux, geralmente é mais rápido!**
...
#### Método 2: Copiar para celular

```bash
# Copiar via ADB
adb push android/app/build/outputs/apk/release/app-release.apk /sdcard/Download/

# Ou copiar para PC e enviar por WhatsApp/Email
cp android/app/build/outputs/apk/release/app-release.apk ~/Downloads/
```

#### Método 3: Transferir via USB

```bash
# Copiar para dispositivo montado
cp android/app/build/outputs/apk/release/app-release.apk /media/$USER/SeuCelular/Download/
```
...
```bash
# Copiar ícones
cp ~/meus-icones/ic_launcher_48.png android/app/src/main/res/mipmap-mdpi/ic_launcher.png
cp ~/meus-icones/ic_launcher_72.png android/app/src/main/res/mipmap-hdpi/ic_launcher.png
# ... e assim por diante
```
...
### Usar ImageMagick no Linux (gerar ícones automaticamente):

```bash
# Instalar ImageMagick
sudo apt install imagemagick

# Gerar todos os tamanhos a partir de uma imagem grande (512x512)
...
### Upload para servidor (Linux específico):

```bash
# Upload via scp
scp android/app/build/outputs/apk/release/app-release.apk user@servidor:/var/www/html/downloads/

# Upload via rsync
rsync -avz android/app/build/outputs/apk/release/app-release.apk user@servidor:/path/
```
...
```bash
ls -la ~/keystores/meu-app-key.keystore
```
...
```bash
# Usar mais threads (se tiver RAM suficiente)
cd android
...
### Monitorar uso de CPU durante build
...
Crie um script `build-release.sh`:
...
# Copiar APK para pasta de releases
mkdir -p releases
cp android/app/build/outputs/apk/release/app-release.apk releases/app-v$(date +%Y%m%d-%H%M%S).apk

echo "✅ Build concluído!"
echo "📦 APK: $(ls -lh releases/*.apk | tail -1)"
```
...
# Assinar APK
apksigner sign --ks ~/keystores/meu-app-key.keystore --out app-signed.apk android/app/build/outputs/apk/release/app-release-unsigned.apk
...
Você agora sabe criar um APK completo do seu aplicativo React Native no Linux!

**Próximo passo:** Vá para **08-resumo-completo-linux.md** para um guia de referência rápida de todos os comandos!

---

## 💡 Vantagens de Build no Linux

- ✅ **20-30% mais rápido** que Windows
- ✅ **Menos problemas com permissões**
- ✅ **Scripts bash** para automatização
- ✅ **Melhor uso de cache**
- ✅ **Terminal mais poderoso** para debugging

Você está usando a melhor plataforma para desenvolvimento React Native! 🐧🚀
